...
def run_reboot(booster, X_train, y_train, eta, reg_lambda) -> AttackOutput:
    ...
    # Passamos apenas X_train (sem label) como referência.
    #
    # Por quê: formatar_saida_ataque usa X_train para:
    #   (1) tratar infinitos (min/max por coluna)
    #   (2) sortear valores para features desconhecidas
    #   (3) determinar num_features = X_train.shape[1]
    # Ela sempre cria uma linha de tamanho num_features + 1, onde a última
    # coluna é amostra['label'].
    #
    # Se passássemos concat(X_train, y_train) [M+1 cols], ela criaria M+2
    # colunas: M features + reconstrução-do-label-como-feature + label real.
    # Passar só X_train [M cols] produz exatamente M+1 colunas:
    #   cols[:M]  = features reconstruídas
    #   col[M]    = amostra['label'] = label inferido pelo REBOOT
    amostras_formatadas = np.asarray(
        formatar_saida_ataque(amostras_reconstruidas, X_train),
        dtype=float,
    )

    expected_cols = M + 1
    actual_cols = amostras_formatadas.shape[1]
    if actual_cols != expected_cols:
        raise ValueError(
            f"run_reboot: amostras_formatadas tem {actual_cols} colunas, "
            f"esperava {expected_cols} ({M} features + 1 label). "
            "Verifique a assinatura de formatar_saida_ataque."
        )

    return AttackOutput(
        X_rec=amostras_formatadas[:, :M],   # features
        y_rec=amostras_formatadas[:, M],    # amostra['label']
        attack_time_s=time.time() - t0,
        info={"n_reconstructed": len(amostras_formatadas)},
    )
