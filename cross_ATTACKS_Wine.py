...
from sklearn.datasets import load_breast_cancer, fetch_openml, load_wine
...
from REBOOT.metrics.metrics import calcular_ra, calcDistanceReconstruct, createScatterPlotReconstruction
...
def load_target_dataset(dataset_name):
    ...
    if dataset_name == "breast_cancer":
        ...
        # Carrega o wine dataset do sklearn
        data = load_wine(as_frame=True)
        X, y = data.data, data.target
        # Transforma em classificação binária: Classe 0 se torna 1, Classes 1 e 2 se tornam 0
        y = (y == 0).astype(int)
    else:
...
if __name__ == "__main__":
    datasets_to_run = ["pima_diabetes"]
    epsilons = [0.5, 1.0, 5.0]
    kf = RepeatedKFold(n_repeats=10, n_splits=10, random_state=SEED)
